...
class RTDataHandler(RTDataHandlerBase):
    ...
    def on_recv_rsp(self, rsp_str):
        ...
        if ret_code != RET_OK:
            self.default_logger.error("RTDataHandler: error, msg: %s" % data)
            return RET_ERROR, data

        # Column Mapping between Subscribed Data <==> Historical Data
        data = data[
            ['code', 'time', 'cur_price', 'cur_price', 'cur_price', 'cur_price', 'avg_price',
             'avg_price', 'volume', 'turnover', 'avg_price', 'last_close']]
